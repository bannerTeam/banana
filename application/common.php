...
{
    $pattern = "234567890ABCDEFGHIJKLOMNOPQRSTUVWXYZ";
    if($f=='num'){
        $pattern='1234567890';
    }
    $len = strlen($pattern);
...
function mac_rep_pse_rnd($pse,$txt,$id=0)
{
    ...
    if ($j>0){
        $res= mac_substring($txt,$j-1) . $psearr[$id % $i] . mac_substring($txt,strlen($txt)-$j,$j);
    }
    else{
        $res= $psearr[$id % 1]. $txt;
    }
...
{
    $arr = [1=>'视频',2=>'文章',3=>'专题',4=>'评论',5=>'留言',6=>'用户中心'];
    return $arr[$data];
}
...
function mac_get_user_portrait($user_id)
{
    ...
    if(!empty($user_id)){
        $res = MAC_PATH . 'upload/user/'.($user_id % 10 ). '/'.$user_id.'.jpg';
    }
    return $res;
}
...
function mac_play_list($vod_play_from,$vod_play_url,$vod_play_server,$vod_play_note,$flag='play')
{
    ...
    if(BIND_MODULE!='admin' && MAC_PLAYER_SORT=='1'){
        array_multisort($sort, SORT_DESC, SORT_STRING, $res_list);
        $tmp=[];
...
function mac_url_img($url)
{
    ...
    elseif(substr($url,0,4) != 'http'){
        ...
        if($config['upload']['mode']=='1'){
            $url = $config['upload']['remoteurl'] . $url;
        }
        else{
            $url = MAC_PATH . $url;
        }
...
function mac_url_create($str,$type='actor',$flag='vod',$ac='search')
{
    if(!$str){
        return '未知';
    }
    ...
    $arr = explode(',',$str);
    foreach($arr as $k=>$v){
        $res[$k] = '<a href="'.mac_url($flag.'/'.$ac,[$type=>$v]).'" target="_blank">'.$v.'</a>&nbsp;&nbsp;';
    }
    return implode('',$res);
}
...
function mac_label_art_detail($param)
{
    ...
    define('MAC_TYPE_ID',$res['info']['type_id']);
    define('MAC_TYPE_PID',$res['info']['type']['type_id']);
    return $res;
}
function mac_label_vod_detail($param)
{
    ...
    define('MAC_TYPE_ID',$res['info']['type_id']);
    define('MAC_TYPE_PID',$res['info']['type']['type_pid']);
...
function mac_label_type($param)
{
    ...
    define('MAC_TYPE_ID',$type_info['type_id']);
    define('MAC_TYPE_PID',$type_info['type_pid']);
...
{
    if(!in_array($flag,['vod','art'])){
        $flag='vod';
    }
    if(!in_array($range,['all','today'])){
        $range='all';
    }
